...
class Viewer extends SimulationViewer {
	...
	private void initGUI() {
		...
		addKeyListener(new KeyListener() {
			...
			public void keyPressed(KeyEvent e) {
				...
				case 'v':
					if(_showVectors) {_showVectors = false;}
	...
	private void showHelp(Graphics2D g) {
		...
		g.drawString("Scaling ratio: ...", 12, 60);
		if(_selectedGroup == null) {
			String s = new String("Selected Group: " + "all");
			g.setColor(Color.BLUE);
			g.drawString(s, 12, 75);
		}
		else {
			String s = new String("Selected Group: " + _selectedGroup);
			g.setColor(Color.BLUE);
			g.drawString(s, 12, 75);
		}
	...
	private void drawBodies(Graphics2D g) {
		...
		for(Body i : _bodies) {
			if(isVisible(i)) {
				int x = _centerX + (int)(i.getPosition().getX()/_scale);
				int y = _centerY + (int)(i.getPosition().getY()/_scale);
				g.setColor(_gColor.get(i.getgId()));
				g.fillOval(x, y, 8, 8);
				g.setColor(Color.BLACK);
				g.drawString(i.getId(), x, y-5);
				if(_showVectors) {
					//velocity
					int fx = x + (int)(i.getVelocity().direction().getX() * 15);
					int fy = y + (int)(i.getVelocity().direction().getY() * 15);
					drawLineWithArrow(g, x, y, fx, fy, 5, 5, Color.GREEN, Color.GREEN);
					//force
					fx = x + (int)(i.getForce().direction().getX() * 15);
					fy = y + (int)(i.getForce().direction().getY() * 15);
